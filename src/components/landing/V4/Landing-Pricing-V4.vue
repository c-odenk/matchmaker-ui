<template>
  <section ref="pricingSection" id="pricing" class="py-12 md:py-16 lg:py-20 bg-white">
...
export default {
  ...
  data() {
    return {
      // Standardwerte werden sofort angezeigt und – falls erreichbar – vom API überschrieben.
      plansLoading: false,
      selectedTier: { label: 'Bis zu 50 Talente', price: '129,00€', priceSuffix: ' / Monat' },
      starter: {
  ...
  methods: {
    applyPlans(data) {
      ...
      // Bisherige Auswahl (per Label) in den neuen Tiers wiederfinden, sonst ersten Tier wählen.
      const match = this.pro.tiers.find(t => t.label === this.selectedTier.label)
      this.selectedTier = match || this.pro.tiers[0]
    },
    async fetchPlans() {
      try {
