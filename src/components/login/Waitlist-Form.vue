      ...
      <div class="waitlist-card w-full">
        <div class="rounded-2xl bg-white overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.4)]">
          ...
          <div v-if="submitted" key="success-view" class="px-6 sm:px-10 py-16 flex flex-col items-center text-center gap-6">
            <div class="w-16 h-16 rounded-2xl bg-blue flex items-center justify-center shadow-lg shadow-blue/20 text-white">
            ...
            <router-link to="/" class="inline-flex items-center gap-2 rounded-xl px-6 py-3 text-p-small-sm font-semibold text-white bg-blue hover:shadow-lg transition-all duration-200">
              Zurück zur Startseite
            </router-link>
          </div>
          ...
          <div v-else key="form-view" class="px-6 sm:px-8 py-10">
            ...
            <form @submit.prevent="handleSubmit" class="flex flex-col gap-5">
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-[11px] font-bold text-gray-400 uppercase tracking-wider ml-1">Unternehmen</label>
                <input v-model="form.company" type="text" placeholder="Muster GmbH" class="waitlist-input" required />
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="text-[11px] font-bold text-gray-400 uppercase tracking-wider ml-1">E-Mail</label>
                <input v-model="form.email" type="email" placeholder="name@unternehmen.de" class="waitlist-input" required />
              </div>
              <button
                type="submit"
                :disabled="isLoading"
                class="waitlist-btn w-full flex items-center justify-center gap-3 rounded-xl px-6 py-3.5 text-p-sm text-white bg-blue transition-all duration-200 mt-2"
              >
